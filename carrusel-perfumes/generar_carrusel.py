...
def construir(clave):
    ...
    caja_destacada(img, cx(d, t, fb), y + 20, t, fb, bg=caja_bg, fg=caja_fg)
    d = ImageDraw.Draw(img)
    ...
    img = foto_cover(os.path.join(FOTOS, "cielo.jpg"), ANCHO, ALTO, desenfoque=2)
    ...
    img = foto_cover(os.path.join(FOTOS, "cielo2.jpg"), ANCHO, ALTO)
    ...
    img = P["tapa"]((0.5, 0.7)) if clave != "foco_clasico" else P["tapa"]()
    d = ImageDraw.Draw(img)
    fg1 = inter(800, 64)
    d.text((cx(d, "Guarda este", fg1), 150), "Guarda este", font=fg1, fill=P["tapa_txt"])
    fp_ = inter(900, 150)
    w = d.textlength("post", font=fp_)
    d.rectangle((ANCHO / 2 - w / 2 - 70, 250, ANCHO / 2 + w / 2 + 70, 410), fill="#E9B8D3" if clave == "rosa_celeste" else caja_bg)
    d.text((cx(d, "post", fp_), 235), "post", font=fp_, fill=AZUL_NOCHE if clave == "rosa_celeste" else caja_fg)
    destello(d, ANCHO / 2 - w / 2 - 70, 400, 60, ROJO if clave == "rosa_celeste" else BLANCO)
    fs_ = inter(700, 36)
    lineas(d, 0, 450, "y comenta tu perfume favorito\npara incluirlo en la parte II.", fs_, P["tapa_txt"], 50, centrado=True)
    foto = foto_cover(os.path.join(FOTOS, "mano_perfume2.jpg"), 560, 620)
    pegar_redondeado(img, foto, (ANCHO - 560) // 2, 600, radio=36)
    meta(img, 7, P["tapa_meta"]); img.save(out + "slide_07_cta.png")
